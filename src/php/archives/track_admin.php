...
include 'db.php';
    ...
    if ($userId !== null) {
        $stmt = $conn->prepare("INSERT INTO admin_tracking_url  (user_id, visit_time, ip_address, user_agent, referer, page_url, method, session_id, load_time, cookies) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if (headers_sent()) {
            ...
            header("Location: ./verify_admin.php?token=$token");
            exit;
        }
