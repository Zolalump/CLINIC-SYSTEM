...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['check-email'])) {
        ...
        if (count($errors) === 0) {
            include __DIR__ . '/send_email_admin.php';
        ...
        if (isset($_POST['reset_code'])) {
            ...
            if (empty($resetCode)) {
                ...
                $stmt = $conn->prepare('SELECT * FROM admin_password_resets WHERE reset_code = ?');
                ...
                if ($reset) {
                    header('Location: /WebDa/booking/src/php/reset_password_form_adm.php?reset_code=' . $resetCode);
                    exit();
                } else {
...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST" autocomplete="off" class="mb-6">
            <div class="mb-6">
